...
async function registerSlashCommands(client) {
  ...
  try {
    ...
    const warn = registered.find(c => c.name === 'warn');
    if (warn) {
      logger.info(
        `/warn default_member_permissions=${warn.default_member_permissions} ` +
        `(requires Kick, Ban, or Time out members on the role)`
      );
    }
  } catch (err) {
